...
To solve this problem, this paper proposes LDA-DE: a combination of LDA and a
Search-Based Software Engineering method
(Differential Evolution) that automatically tunes LDA's
$<k,\alpha,\beta>$ parameters. LDA-DE is tested on  data from a programmer
...
To fix this problem,
we proposes LDA-DE: a  combination of LDA and a search-based optimizer (differential evolution, or DE)~\cite{storn1997differential})
that automatically tunes LDA's \mbox{$<k,\alpha,\beta>$} parameters. This paper tests LDA-DE on:
\bi
...
\ei
Researchers have suggested that the important parameters which affect LDA the most are those above mentioned. But there is $u$, which is number of update iterations. Since no one has cared about this parameter, we have not used it for tuning.
...
we will study the $\mathit{size\_of\_topic}=9$ but the standard in the LDA literature~\cite{panichella2013effectively, lukins2010bug} says that top 10 terms per topic are ideal. We did not select $\mathit{size\_of\_topic}=10$ just because our results showed that the $\Re_9$ is negligible.
...
\begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1]
    %\label{algo: untuned}
    \Require $Data$, $\mathit{size\_of\_topic}$, $k$, $\alpha$, $\beta$ (Defaults) 
    \Ensure $Raw\_Score$    
    \Function{ldascore}{ $\mathit{size\_of\_topic}$, $Data$}
        ...
        \For{$j = 0$ to 10}
            ...
            \EndFor
            \State $Score$.append($Overlap$(Topics, $\mathit{size\_of\_topic}$, $l[0]$))
        \EndFor
...
\begin{algorithm}[!t]
    ...
    \Function{DE}{$np,f,cr,iter, Goal$}
        ...
        \For{$i = 0$ to $np-1$}
            \State $Cur\_Gen$.append($Population$[i],ldascore($Population$[i], $\mathit{size\_of\_topic}$, $Data$)
        \EndFor
        \For{$i = 0$ to $iter$}
            ...
            \For{$j = 0$ to $np-1$}
                ...
                \If{ldascore($S_i$) $\geq$ $Cur\_Gen$[j][1]}
                    \State $NewGeneration$.append($S_i$,ldascore($S_i$,$\mathit{size\_of\_topic}$, $Data$))
                \Else
...
\subsection{Tuning Topic Modeling with LDA-DE}
\label{sect: tuning}
LDA-DE is a combination of topic modeling (with LDA) and an optimizer (differential evolution, or DE) that adjusts
...
%\newpage
LDA-DE  adjusts the parameters of
...
Algorithm 2 shows LDA-DE's version of DE.  DE evolves a \textit{NewGeneration} of
...
parameters. Note that in Figure \ref{RQ3:k}, IQR=0 for  PitsB dataset where tuning
...
%YYY why so fast? io bound?
  Figures \ref{RQ5 VEM} and \ref{RQ5 Gibbs} are shown in blue, red the runtimes required to run LDA untuned, tuned (respectively).
