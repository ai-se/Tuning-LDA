...
\tiny
  \centering
    \begin{tabular}{|c@{~}|c@{~}|c@{~}|c@{~}|c@{~}|c@{~}|c@{~}|p{8cm}@{~}|p{2.5cm}|}
        \hline 
...
\item \textbf{RQ1}: \textbf{Are the default settings of LDA misleading?} We will show that using the default settings of LDA for SE data can lead to systematic errors since stability scores start to drop after $n=5$ terms per topic.
    ...
    \item \textbf{RQ4}: \textbf{Is tuning easy?} We show that, measured
      ...
      tuning LDA is much simpler that standard optimization methods.
...
we apply LDADA to over 9000 SE conference publications to find trends in topics
 1993 to 2013.
That paper reports (a) what topics that are
 becoming more/less prominent; (b) what topics  are most frequently
 accepted to what conferences in our corpus; (c) which conferences mostly
...
Thirdly, while this specifics of this paper are about taming instability in topic modeling,
...
purposes that LDADE. That worked uses GAs to increase the precision
...
Topic modeling on StackOverflow is useful for finding patterns in programmer knowledge.
 ...
 sources\footnote{https://archive.org/details/stackexchange}. 
  ...
  \textbf{Citemap} contains titles and abstracts of over 9000 papers from a
...
\item Topic~3 of run1 scores $\frac{4}{4}=1$.
  ...
  since
  any report of the contents of a topic that uses more that two words per topic would be unreliable.
...
\item The inter-quartile range (IQR) of those tunings (the IQR is the 75th-25th percentile values and is a non-parametric measure of variation
  ...
  \ei
  These figures
clearly show how tuning selects the different ranges of values of
...
To understand our timing results, recall that untuned/tunes LDA use
Algorithm~1/ Algorithm~2 respectively. In turns out that some of the
book keeping required for the data collection of Algorithm~1 can be
spread over Algorithm~2, after which we expect that Algorithm~2 will
run ten times about slower that Algorithm~1. 
  ...
  seven, but often less than five. Why is this slowdown less that our expected value of ten?
  The answer is in the relavtive times required for LDA and DE: the latter is much slower
  than the former which means variances in the performance of LDA dominate these timing results.

  Regardless of whehter or not the slow down is a factor of five or seven or ten,
