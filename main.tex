...
\ei
%Various studies done by researchers given in Table~\ref{tbl:survey2} have suggested that the important parameters which affect LDA the most are those above mentioned. But there is $u$, which is number of update iterations. Since no one has cared about this parameter, we have not used it for tuning.
...
Also,
  ...
  can improve stability for different platforms and implementations for LDA.
...
holds true in practice. Shown in blue and red are the
  ...
  seven, but often less than five (which is very close to our theoretical prediction).
  Hence, we say:

  
  \begin{lesson}
    Theoretically and empirically, tuning LDA costs three to seven times more runtime
