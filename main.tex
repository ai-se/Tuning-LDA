...
across different implementations of LDA (Python+ScikiLearn, Scala+Spark); across
...
which are then used by, say, a classifier to characterize different kinds of software (e.g. buggy or not\footnote{need papers her on LDA and supervised learning}).
...
\begin{compactitem}
\item \textbf{RQ1}: \textit{Are the default settings of LDA misleading?} We will show that topics generated with the default settings are misleading since  stability scores start to drop after using $k=5$ terms per topic.
...
we also think that this work makes a more general point.
      ...
      adjusting the control parameters of an algorithm
      usied in software analytics. Previously~\cite{fu2016tuning},
...
\begin{figure*}[!t]
  \begin{center}
    ...
    \end{center}
  \caption{Example of terms overlap off size $n=4$ across multiple runs.}
  \label{fig:jaccard}
\end{figure*}
...
NASA projects, which we label as PitsA, PitsB, etc.
 ...
 programming.
Topic modeling on StackOverflow is useful for finding patterns in programmer knowledge\footnote{no SO restructions? python? we did it all?}
 This data can be downloaded from various
 sources\footnote{http://data.stackexchange.com.
...
\bi
\item
  Stop words removal: ignore very common short words such as  ``and'' or ``the'' \footnote{AA: got a url for the stop list you used}
\item
...
\item PITS and Citemap is small enough to process on a single (four core) deskop machine
 ...
 This study used a Spark and Mllib on a cluster of 50 nodes to
 reduce the runtime.
 \ei
...
we will study the $k=10$ most likely topics since that is standard in
the LDA literature~\cite{panichella2013effectively, lukins2010bug}.

Next, for $1 \le n \le k$, we will calculate the median size of the overlap,
computed as follows:
\bi
...
\item The \textbf{\textit{Delta score}} is the difference between the two
  ...
  every 10 runs, and this process is continued 10 times. At the end, the median
...
extrapolating$^{31}$ between current solutions in the frontier.. Three solutions a, b, c are
...
Also,
  ...
  based on Mllib running on a 50 node Spark system (8 cores per node).
...
Note that the  stability collapses after $n=5$ words. This means
  ...
  be changed, just be changing the order of the inputs. This is a significant result
...
relating to other data sets are shown at 
https://goo.gl/HQNASF.
...
the tunings required for software clone detectors~\cite{Wang:Harman13}. Sayyad et al. routinely used $10^6$ evaluations (or more)
  of their models in order to extract products from highly constrained product lines~\cite{Sayyad2013}. Hence, before recommending
...
\subsection{\textbf{RQ6: Should data miners be used “off-the-shelf” with their  default  tunings?}}
  ...
  Figure~\ref{fig:deltas} shows that there is much benefit in tuning.
\item
...
\item
  ...
  a class of papers that do {\em not} do that. Rather, they use LDA as part of some internal
  process where the topics are intermediaries
  used to generate some other goal\footnote{amrith: apper). As we said at the start
...
other researchers to repeat/ refute/ improve their conclusions\footnote{amrith: can we make
  all the footnotes of this paper be urls where folsk can download materials relevant to this paper}. For example, the footnotes of this contain all the urls where other researchers can download
...
Another threat to validity of this work is that it is a qurk of the control
...
evaluation budget of 300 evaluations\footnote{amrit? true?}.  Table~V
compares results across different numbers of evaluations. While clearly,
the more evaluations the better, there is little improvement after 300 evalautions.
