...
\item The \textbf{\textit{Delta score}} is the difference between the two
  ...
  score is selected as the untuned raw score ($\Re_n$ ) $^{3-11}$. Here, the runtimes comes from $10 * 10$ evaluations of untuned experiment.
...
\begin{algorithm}[!t]
    ...
    \Function{ldascore}{ $n$, $Data$}
        ...
        \For{$j = 0$ to 10}
            ...
            \EndFor
            \State $Score$.append($Overlap$(Topics, $n$, $l[0]$))
        \EndFor
...
\begin{algorithm}[!t]
    ...
    \Function{DE}{$np,f,cr,iter, Goal$}
        ...
        \For{$i = 0$ to $np-1$}
            \State $Cur\_Gen$.append($Population$[i],ldascore($Population$[i], $n$, $Data$)
        \EndFor
        \For{$i = 0$ to $iter$}
            ...
            \For{$j = 0$ to $np-1$}
                ...
                \If{ldascore($S_i$) $\geq$ $Cur\_Gen$[j][1]}
                    \State $NewGeneration$.append($S_i$,ldascore($S_i$,$n$, $Data$))
                \Else
...
come similarly from Alogrithm 1$^{3-11}$. But there is not any outer loop$^{3}$. In Algorithm 2, LDA is only run as 1 rig$^{11 \& 12}$. Here the runtimes comes from $\mathit{iter} * np * 10$ evaluations of tuned experiment.
...
Our results show that before tuning, after 5 words, that report is misleading.
After tuning, it is legitimate to report up to after around eight words per topic.
